...
def render_annotated_image_with_modal(raw_bytes, annotated_np, crops_data, detections):
    ...
    for i, (det, cd) in enumerate(zip(detections, crops_data)):
        ...
        bbox_zones += (
            ...
            f'onclick="event.stopPropagation();openModal({i})" '
...
def render_metrics(total, hi, me, lo, vl):
    return f"""<div class="metrics-row">
      ...
      <div class="metric metric-vlow"><div class="metric-val">{vl}</div><div class="metric-lbl">&lt;20%</div></div>
    </div>"""
...
.sb-logo-mark{width:36px;height:36px;flex-shrink:0;background:linear-gradient(135deg,var(--blue),var(--blue-dark));border-radius:10px;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 12px rgba(108,99,255,.3);}
...
.sb-stage{display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:8px;background:var(--surface-2);border:1px solid var(--border);}
.sb-stage-num{width:22px;height:22px;border-radius:4px;background:var(--blue);color:#fff!important;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
...
.status-pill{display:inline-flex;align-items:center;gap:7px;padding:8px 12px;border-radius:8px;font-size:11px;font-family:var(--font-mono);font-weight:500;}
...
.page-header-title{font-size:22px;font-weight:800;background:linear-gradient(90deg,var(--text),var(--blue));-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin:0 0 2px;}
.page-header-sub{font-size:12px;color:var(--text-muted);margin:0;}
...
.section-title{font-size:13px;font-weight:800;color:var(--text);margin:0 0 10px;}
...
.det-card.ch{border-left-color:var(--green);}.det-card.cm{border-left-color:var(--amber);}
.det-card.cl{border-left-color:var(--orange);}.det-card.cv{border-left-color:var(--red);}
...
.det-expand-inner{display:grid;grid-template-columns:96px 1fr;gap:12px;padding-top:12px;}
.det-large-img{width:96px;height:96px;object-fit:contain;border-radius:6px;background:var(--surface-2);padding:4px;border:1px solid var(--border);}
.det-fields{display:flex;flex-direction:column;gap:3px;}
.det-field{display:flex;align-items:flex-start;gap:6px;font-size:11px;}
.det-fk{color:var(--text-muted);width:68px;flex-shrink:0;font-family:var(--font-mono);font-size:10px;}
.det-fv{color:var(--text);font-weight:600;font-size:11px;}
.conf-bar-wrap{margin-top:8px;}
.conf-bar-lbl{font-size:10px;color:var(--text-muted);font-family:var(--font-mono);margin-bottom:4px;}
.conf-bar-track{height:5px;background:var(--border-2);border-radius:2px;overflow:hidden;}
...
.tbl-wrap{background:var(--surface);border:1px solid var(--border);border-radius:var(--r);overflow:auto;max-height:320px;box-shadow:var(--sh);}
...
.inv-card-img{width:100%;height:110px;object-fit:contain;background:var(--surface-2);border-radius:var(--r-sm);margin-bottom:8px;border:1px solid var(--border);display:block;}
.inv-card-name{font-size:11px;font-weight:600;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:2px;}
.inv-card-sku{font-size:9px;color:var(--text-muted);font-family:var(--font-mono);margin-bottom:4px;}
.inv-card-conf{font-size:10px;font-weight:700;margin-bottom:6px;}

/* ═══════ CAMERA ═══════ */
.cam-wrap{background:var(--surface);border:1px solid var(--border);border-radius:var(--r-lg);padding:20px;box-shadow:var(--sh);text-align:center;}
...
# PAGE HEADER
# ══════════════════════════════════════════════════════
st.markdown('<div class="page-header"><div class="page-header-title">SKU Recognition Pipeline</div><div class="page-header-sub">YOLO + MobileNetV3 · Détection et identification de produits en rayon</div></div>',unsafe_allow_html=True)
...
with tab_inv:
    ...
    with inv_cam:
        ...
        if not st.session_state.inv_cam_active:
            ...
            with ic1:st.button("▶",key="inv_s2",disabled=True,use_container_width=True)
            with ic2:
    ...
    if validated:
        ...
        st.markdown('<div class="empty-state"><div class="empty-title">Inventaire vide</div><div class="empty-sub">Validez des produits depuis la section ci-dessus</div></div>',unsafe_allow_html=True)